...
use v3::{value, values::Value, units::{UnitLength, UnitSubstance}, units::{Metric, UnitAngle}};
...
fn criterion_benchmark(c:&mut Criterion) {
    ...
    c.bench_function("convert_direct", |b| b.iter(|| black_box(va3_2>>=UnitLength::Foot)));
    c.bench_function("unit_conversion km   - mile", |b| b.iter(|| black_box(u1.convert(&u2))));
    c.bench_function("unit_conversion mile - km", |b| b.iter(|| black_box(u2.convert(&u1))));
    c.bench_function("unit_conversion km - mm", |b| b.iter(|| black_box(u1.convert(&u3))));
