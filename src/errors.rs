...
mod value_errors_testing {
    ...
    #[test]
    fn static_errors() {
        assert_eq!(format!("{}", V3Error::UnitConversionError), "Unit Conversion Error");
    }

    #[test]
    fn str_errors() {
        assert_eq!(format!("{}", V3Error::ParsingError("parse")), "Parsing error: parse");
        assert_eq!(format!("{}", V3Error::UnknownError("unknown")), "Unknown Error: unknown");
        assert_eq!(format!("{}", V3Error::ValueConversionError("conversion")), "Value Conversion Error: conversion");
    }
    ...
    #[test]
    fn debug_string() {
        assert!(format!("{:?}", V3Error::ParsingError("t1")).chars().count() > format!("{}", V3Error::ParsingError("t1")).chars().count());
    }
}