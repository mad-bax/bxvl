...
use crate::{
    constants::{LUMINOUS_FLUX_INDEX, LUMINOUS_FLUX_MAP},
    errors::V3Error,
...
mod conversion_testing {
    ...
    use crate::units::{Metric, UnitLength, UnitLuminousFlux};
    ...
    fn covert_mut_fail() {
        ...
        x >>= UnitLuminousFlux::Lumen(Metric::None);
    }

    #[test]
    fn covert_stat_fail() {
        assert!(((1.0 * UnitLength::Foot) >> UnitLuminousFlux::Lumen(Metric::None)).is_err());
    }

    #[test]
    fn unit_conversions_stat_exp1() {
        let t1 = 4.1 * UnitLuminousFlux::Lumen(Metric::None);

        for i in TEST_METRIC {
            // Scale the metric value by converting
            let t2 = (t1 >> UnitLuminousFlux::Lumen(i.0)).unwrap();
    ...
    fn unit_conversions_mut_exp1() {
        let mut t1 = 4.1 * UnitLuminousFlux::Lumen(Metric::None);
        let t1_orig = 4.1 * UnitLuminousFlux::Lumen(Metric::None);

        for i in TEST_METRIC {
            // Scale the metric value by converting
            t1 >>= UnitLuminousFlux::Lumen(i.0);
            ...
            // Reset a little bit
            t1 = 4.1 * UnitLuminousFlux::Lumen(Metric::None);
        }
    }
}
