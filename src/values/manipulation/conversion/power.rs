...
mod conversion_testing {
    ...
    use crate::units::{Metric, UnitLength, UnitPower};
    ...
    fn covert_mut_fail() {
        ...
        x >>= UnitPower::Watt(Metric::None);
    }

    #[test]
    fn covert_stat_fail() {
        assert!(((1.0 * UnitLength::Foot) >> UnitPower::Watt(Metric::None)).is_err());
    }
    ...
    fn unit_conversions_stat_exp1() {
        ...
        for i in TEST_METRIC {
            ...
            let t2 = (t1 >> UnitPower::Watt(i.0)).unwrap();
    ...
    fn unit_conversions_mut_exp1() {
        let mut t1 = 4.1 * UnitPower::Watt(Metric::None);
        let t1_orig = 4.1 * UnitPower::Watt(Metric::None);

        for i in TEST_METRIC {
            // Scale the metric value by converting
            t1 >>= UnitPower::Watt(i.0);
            ...
            // Reset a little bit
            t1 = 4.1 * UnitPower::Watt(Metric::None);
        }
    }
}
